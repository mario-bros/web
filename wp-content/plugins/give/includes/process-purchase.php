...
function give_process_purchase_form() {
	...
	$give_amount    = isset( $_POST["give-amount"] ) ? trim( $_POST["give-amount"] ) : false;
	
	//echo "test " . $give_amount;
	
	$is_ajax = isset( $_POST['give_ajax'] );
	if ($give_amount == 0.00 || $give_amount == 0 || !$give_amount || $give_amount == "" || $give_amount == "0.00" || $give_amount < 1) {
	give_set_error( 'email_empty', __( 'Please choose an amount first', 'give' ) );	
	...
	//$donation = new wc_quick_donation();
	//$donation->process_donation($_POST);
	//give_die();
...
function give_purchase_form_validate_logged_in_user() {
	...
	if ( $user_ID > 0 ) {
		...
		$give_purchase = give_purchase_form_required_fields();

		if (isset($give_purchase)) {
		// Loop through required fields and show error messages
			foreach ( $give_purchase as $field_name => $value ) {
