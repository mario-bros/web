...
class WC_Gateway_Buckaroo_SepaDirectDebit extends WC_Gateway_Buckaroo {
    ...
    function process_payment($order_id) {
            ...
            if (!$sepadirectdebit->isIBAN($_POST['buckaroo-sepadirectdebit-iban'])){
                wc_add_notice( __("Wrong IBAN number", 'woocommerce'), 'error' );
    ...
    function payment_fields() {
        ...
        ?>
            ...
            <fieldset>
                ...
                <p class="form-row">
                        <label for="buckaroo-sepadirectdebit-gender"><?php echo _e('Gender:', 'woocommerce')?><span class="required">*</span></label>
                ...
                <p class="form-row form-row-wide validate-required">
                        <label for="buckaroo-sepadirectdebit-birthdate"><?php echo _e('Birthdate:', 'woocommerce')?><span class="required">*</span></label>
                ...
                <?php endif;?>
                <p class="form-row form-row-wide validate-required">
                        <label for="buckaroo-sepadirectdebit-accountname"><?php echo _e('Bank account holder:', 'woocommerce')?><span class="required">*</span></label>
                ...
                <p class="form-row form-row-wide validate-required">
                    <label for="buckaroo-sepadirectdebit-iban"><?php echo _e('IBAN:', 'woocommerce')?><span class="required">*</span></label>
                ...
                <p class="form-row form-row-wide">
                        <label for="buckaroo-sepadirectdebit-bic"><?php echo _e('BIC:', 'woocommerce')?></label>
	...
	public function response_handler() {
        ...
        function init_form_fields() {
            ...
            $this->form_fields['usecreditmanagment'] = array(
                                'title' => __( 'Use Credit Managment', 'woocommerce' ), 
                                'type' => 'select', 
                                'description' => __( 'Buckaroo sends payment reminders to the customer. (Contact Buckaroo before activating Credit Management. By default this is excluded in the contract.)', 'woocommerce' ), 
                                'options' => array('TRUE'=>'Yes', 'FALSE'=>'No'),
                                'default' => 'FALSE');
            $this->form_fields['invoicedelay'] = array(
                                'title' => __( 'Invoice delay (in days)', 'woocommerce' ), 
                                'type' => 'text', 
                                'description' => __( 'Specify the amount of days before Buckaroo invoices the order and sends out the payment mail.', 'woocommerce' ), 
                                'default' => '3');
            $this->form_fields['datedue'] = array(
                                'title' => __( 'Due date (in days)', 'woocommerce' ), 
                                'type' => 'text', 
                                'description' => __( 'Specify the number of days the customer has to complete their payment before the first reminder e-mail will be sent by Buckaroo.', 'woocommerce' ), 
                                'default' => '14');           
            $this->form_fields['maxreminderlevel'] = array(
                                'title' => __( 'Max reminder level', 'woocommerce' ), 
                                'type' => 'select', 
                                'description' => __( 'Select the maximum reminder level buckaroo will use.', 'woocommerce' ), 
            ...
            $this->form_fields['paymentmethodssdd'] = array(
                                ...
                                'type' => 'multiselect',
                                'css' => 'height: 650px;',
                                'description' => __( 'Select allowed payment methods for SEPA Direct Debit. (Ctrl+Click select multiple)', 'woocommerce' ),
                                'options' => array(
                                    'ideal' => 'iDEAL',
