	...
	public function response_handler() {
        ...
        function init_form_fields() {
            ...
            $this->form_fields['datedue'] = array(
                                'title' => __( 'Number of days till order expire', 'woocommerce' ), 
                                'type' => 'text', 
                                'description' => __( 'Number of days to the date that the order should be payed.', 'woocommerce' ), 
                                'default' => '14');
            $this->form_fields['sendmail'] = array(
                                'title' => __( 'Send Email', 'woocommerce' ), 
                                'type' => 'select', 
                                'description' => __( 'Buckaroo sends an email to the customer with the payment procedures.', 'woocommerce' ), 
                                'options' => array('TRUE'=>'Yes', 'FALSE'=>'No'),
                                'default' => 'FALSE');
            $this->form_fields['showpayproc'] = array(
                                'title' => __( 'Show payment procedures', 'woocommerce' ), 
                                'type' => 'select', 
                                'description' => __( 'Show payment procedures on Thank You page after payment confirmation.', 'woocommerce' ), 
                                'options' => array('TRUE'=>'Yes', 'FALSE'=>'No'),
                                'default' => 'FALSE');
        } 
}