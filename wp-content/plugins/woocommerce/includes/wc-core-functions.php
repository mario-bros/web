...
function wc_create_order( $args = array() ) {
	...
	if ( ! $updating ) {
		...
		update_post_meta( $order_id, '_recurring_profile', $_SESSION['recur_period']);	
		update_post_meta( $order_id, '_edu_plan', $_SESSION['edu_plan']);	
		update_post_meta( $order_id, '_liv_plan', $_SESSION['liv_plan']);	
	}
	...
	$edusponsorid = "";
	$edusponsorinfo = $_SESSION['edu_plan'];
	$eduinfo = explode("*",$edusponsorinfo);
	$edusponsorid = $eduinfo[1];
	
	if ($edusponsorid != "") {
		update_post_meta( $edusponsorid, '_give_edu_sponsor', $args['customer_id'] );
	}
	
	$livsponsorid = "";
	$livsponsorinfo = $_SESSION['liv_plan'];
	$livinfo = explode("*",$livsponsorinfo);
	$livsponsorid = $livinfo[1];
	
	if ($livsponsorid != "") {
		update_post_meta( $livsponsorid, '_give_liv_sponsor', $args['customer_id'] );
	}	
