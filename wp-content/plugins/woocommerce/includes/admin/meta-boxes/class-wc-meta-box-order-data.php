...
<?php							
		...
		<div class="panel-wrap woocommerce">
			...
			<div id="order_data" class="panel">	
			<h2><?php printf( __( 'Transaction Details', 'woocommerce' ), esc_html( $order->get_order_number() ) ); ?></h2>
...
<?php 
  ...
  $qry = "SELECT * FROM wp_woocommerce_buckaroo_transactions_info where userid = '$user_id' and wc_orderId = '$order' ORDER BY LastUpdate desc";
...
<tr class="head"><td>Date</td><td>Payment Method</td><td>Amount</td><td>TransactionID</td><td>Status</td></tr>
<?php	  
foreach( $states as $row ) {
$_paid_date = $row->LastUpdate;
$payment_method = $row->brq_payment_method;
$payment_id = $row->transaction;
$brq_amount = $row->brq_amount;
$statuscode = $row->brq_statuscode;
	$status = $row->brq_statusmessage;
	$class = "pending";
if ($statuscode == "190") {
	$status = $row->brq_statusmessage;
	$class = "complete";
}
if ($statuscode == "490" || $statuscode == "690") {
	$status = $row->brq_statusmessage;
	$class = "failed";	
}
...
?>
<tr><td><?php echo $_paid_date; ?></td><td><?php echo $payment_method; ?></td><td class="amount">&euro;<?php echo $brq_amount; ?></td><td class="amount"><?php echo $payment_id; ?></td><td class="<?php echo $class; ?> donation"><?php echo $status; ?></td></tr>
<?php
}
?>
</table>	
<style>
.transactionstable { margin:20px; border:0px !important; width:95%; min-width:720px;  }
.transactionstable tr, .transactionstable td { border:0px !important; }
.transactionstable td { border-bottom:1px solid #ccc !important; }
.transactionstable td.donation { width:40%;  }
.transactionstable td.amount { color:#728F72 !important; }
.transactionstable td.complete { color:#728F72 !important; }
.transactionstable td.failed { color:#F00 !important; }
.transactionstable td.pending { color:#C8AE6A !important; }

.transactionstable tr.head { background-color:#F4F4F4; border:none; font-weight:700;  }
.transactionstable tr.head td { text-align:left; padding: 15px; color:#000 !important; border-bottom:0px !important;  }
.transactionstable td { text-align:left; padding: 15px; color:#999 !important;  }
