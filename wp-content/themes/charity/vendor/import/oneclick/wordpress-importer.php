...
if (class_exists('WP_Importer')) {
    ...
    class WP_Import extends WP_Importer {
        ...
        /**
         * Function to update loader status
         */
        function update_import_status($loaded) {
            echo '<<' . $loaded . '>>';
            flush();
            ob_flush();
            sleep(1);
         ...
         */
        function bb_get_author_mapping() {
        ...
        function get_author_mapping() {
            ...
            foreach ($this->authors as $_login => $author) {
                
                //already user
                if( $current_user->user_login == $_login || $current_user->user_email == $author['author_email']){ continue; }
                
                //create new user
                $santized_old_login = sanitize_user($_login, true);
                ...
                if (!is_wp_error($user_id)) {
                    ...
                    //printf(__('Failed to create new user for %s. Their posts will be attributed to the current user.', 'wordpress-importer'), esc_html($author['author_display_name']));
                    if (defined('IMPORT_DEBUG') && IMPORT_DEBUG)
                        echo ' ' . $user_id->get_error_message();
                    echo '<br />';
                }
