...
class Widget_Data {
	...
	public static function add_admin_menus() {
		...
		self::$import_page = add_theme_page( 'Widget Settings Import', 'Widget Settings Import', 'manage_options', 'widget-settings-import', array( __CLASS__, 'import_settings_page' ) );
		// export
		...
		self::$export_page = add_theme_page( 'Widget Settings Export', 'Widget Settings Export', 'manage_options', 'widget-settings-export', array( __CLASS__, 'export_settings_page' ) );
			...
			foreach ( $import_widgets as $import_widget ) :
				...
				if ( isset( $current_sidebars[$import_sidebar] ) ) :
					...
					if ( array_key_exists( $title, $new_widgets ) ) {
						...
						$current_multiwidget = (!empty($current_widget_data['_multiwidget']))? $current_widget_data['_multiwidget']: "";
	...
	public static function get_widget_settings_json() {
		...
		if( isset( $widget_settings['error'] ) )
			return new WP_Error( 'widget_import_upload_error', $widget_settings['error'] );
                //global $wp_filesystem;
                //$fileGetContent=$wp_filesystem->get_contents($widget_settings['file']);

		$file_contents = file_get_contents( $widget_settings['file'] );
                //$file_contents =$fileGetContent;
