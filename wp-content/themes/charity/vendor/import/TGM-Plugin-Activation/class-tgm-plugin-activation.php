    ...
    class TGM_Plugin_Activation {
        ...
        public function admin_init() {
            ...
            if ( isset( $_REQUEST['tab'] ) && 'plugin-information' == $_REQUEST['tab'] ) {
                //requ ire_once ABSPATH . 'wp-admin/includes/plugin-install.php'; // Need for install_plugin_information().
                do_action("bb_require_include", ABSPATH . 'wp-admin/includes/plugin-install.php' );
        ...
        public function install_plugins_page() {
            ...
            <div class="tgmpa wrap">
                ...
                <?php if ( version_compare( $this->wp_version, '3.8', '<' ) ) {
                    ...
                    //sc reen_icon( apply_filters( 'tgmpa_default_sc reen_icon', 'themes' ) );
                } ?>
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                do_action("bb_require_include", ABSPATH . 'wp-admin/includes/plugin-install.php' );
                do_action("bb_require_include", ABSPATH . 'wp-admin/includes/class-wp-upgrader.php' );

                //req uire_once ABSPATH . 'wp-admin/includes/plugin-install.php'; // Need for plugins_api.
                //req uire_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php'; // Need for upgrade classes.
                ...
                $url   = add_query_arg( array( 'action' => 'install-plugin', 'plugin' => $plugin['slug'] ), esc_url('update.php') );
                if ( isset( $_GET['from'] ) ) {
                    $url .= add_query_arg( 'from', urlencode( stripslashes( $_GET['from'] ) ), esc_url($url) );
                }
                ...
                if ( $this->is_automatic ) {
                    ...
                    if ( is_wp_error( $activate ) ) {
                        ...
                        echo '<p><a href="' . add_query_arg( 'page', $this->menu, esc_url(network_admin_url( 'themes.php' )) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . $this->strings['return'] . '</a></p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, esc_url(network_admin_url( 'themes.php' )) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'tgmpa' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, esc_url(network_admin_url( 'themes.php' )) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'tgmpa' ) . '</a>'  : '';
...
if ( ! class_exists( 'WP_List_Table' ) ) {
    do_action("bb_require_include", ABSPATH . 'wp-admin/includes/class-wp-list-table.php');
   // requ ire_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() ) {
                ...
                $url   = add_query_arg( array( 'page' => TGM_Plugin_Activation::$instance->menu ), esc_url(network_admin_url( 'themes.php' )) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
 ...
 * @since 2.2.0
 */
// && ( isset( $_GET['page'] ) && TGM_Plugin_Activation::$instance->menu === $_GET['page'] ) 
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function bulk_footer() {
                ...
                foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                    if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                        echo '<p><a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, esc_url(network_admin_url( 'themes.php' )) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . TGM_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                        $complete[] = $plugin;
                        break;
                    }
                    // Nothing to store.
